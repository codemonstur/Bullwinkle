...
public abstract class ParseTreeObjectBuilder<T> implements ParseNodeVisitor {
	 ...
	 *   build process
	 */
	public final synchronized T build(Node tree) throws BuildException
	{
		if (tree == null)
		{
			throw new BuildException("The input tree is null");
		}
		m_stack = new ArrayDeque<>();
		try
		{
			preVisit();
	...
	@Override
	public synchronized void visit(Node node) throws VisitException {
		try {
			handleNode(node);
		}
	...
	protected void handleNode(Node node) throws IllegalAccessException, InvocationTargetException
	{
